...
// version: optional setting to unlock wallet for block minting only;
...
// version: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // version: timestamp must not exceed spend time
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // version: not avatar mode
                    {
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    int64 nCombineThreshold = GetProofOfWorkReward(GetLastBlockIndex(pindexBest, false)->nHeight, GetLastBlockIndex(pindexBest, false)->nBits) / 3;
